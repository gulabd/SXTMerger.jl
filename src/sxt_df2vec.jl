...
function sxtevt_df2vec(df::DataFrame)
    ...
    phas= convert(Vector{Int}, vec([df[!,"PHAS1"] df[!,"PHAS2"] df[!,"PHAS3"] df[!,"PHAS4"] df[!,"PHAS5"] df[!,"PHAS6"] df[!,"PHAS7"] df[!,"PHAS8"] df[!,"PHAS9"]]'))
    stts= df[!,"STATUS"]
    pha= df[!,"PHA"]
    grade= df[!,"GRADE"]
    ...
    phaso= convert(Vector{Int},vec([df[!,"PHASO1"] df[!,"PHASO2"] df[!,"PHASO3"] df[!,"PHASO4"] df[!,"PHASO5"] df[!,"PHASO6"] df[!,"PHASO7"] df[!,"PHASO8"] df[!,"PHASO9"]]'))
